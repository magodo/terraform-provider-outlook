...
func main() {
	...
	brandName := f.String("brand-name", "", "The friendly/brand name of this Data Source/Resource (e.g. Mail Folder)")
	resourceId := f.String("resource-id", "", "A Resource ID showing an example of how to Import this Resource")
	...
	if isResource && (resourceId == nil || *resourceId == "") {
		quitWithError("Resource ID must be specified via `-resource-id` when scaffolding for a Resource")
		return
	}
...
func getContent(resourceName, brandName string, resourceId *string, isResource bool) (*string, error) {
	...
	if !isResource {
		for key, ds := range provider.SupportedDataSources() {
			if key == resourceName {
		...
		for key, rs := range provider.SupportedResources() {
			if key == resourceName {
...
func saveContent(resourceName string, websitePath string, content string, isResource bool) error {
	...
	fileName := strings.TrimPrefix(resourceName, "outlook_")
...
type documentationGenerator struct {
	...
	// brandName is the marketing brand name used for this resource (e.g. Mail Folder)
	brandName string

	// resourceName is the name of the resource e.g. `outlook_mail_folder`
	resourceName string
...
func (gen documentationGenerator) requiredFieldsForExampleBlock(fields map[string]*schema.Schema, indentLevel int) string {
	...
	// if we have a "name" field output those first as per convention
